-- Custom
local awful = require("awful") -- Para manejar widgets y layouts
local wibox = require("wibox") -- Para widgets de la barra
local gears = require("gears") -- Para utilidades adicionales
local beautiful = require("beautiful") -- Para temas visuales

local battery_widget = awful.widget.watch("acpi -b", 30, function(widget, stdout)
	local battery_perc = stdout:match("(%d?%d?%d)%%") or "N/A"
	widget.text = "🔋 " .. battery_perc .. "%"
end, wibox.widget.textbox())

local wifi_widget = awful.widget.watch("iwgetid -r", 10, function(widget, stdout)
	local wifi_ssid = stdout:gsub("\n", "")
	if wifi_ssid == "" then
		widget.text = "📡 Desconectado"
	else
		widget.text = "📶 " .. wifi_ssid
	end
end, wibox.widget.textbox())

local bluetooth_widget = awful.widget.watch("bluetoothctl show | grep Powered", 10, function(widget, stdout)
	...
	function update_brightness()
		awful.spawn.easy_async_with_shell("brightnessctl g && brightnessctl m", function(stdout)
			...
			if #values < 2 then
				naughty.notify({
					title = "Error",
					text = "No se pudo obtener el brillo",
					timeout = 2,
					fg = "#ff0000",
				})
				return
			...
			local current, max = values[1], values[2]
	...
	function update_volume()
		awful.spawn.easy_async_with_shell("pamixer --get-volume && pamixer --get-mute", function(stdout)
			...
			naughty.notify({
				title = "Volumen",
				text = icon .. " " .. volume,
				timeout = 1.5, -- La notificación dura 1.5s
				replaces_id = 1234, -- Para que no se acumulen muchas notificaciones
			})
		end)
	end
...
globalkeys = gears.table.join(
	...
	awful.key({}, "XF86MonBrightnessUp", function()
		...
		update_brightness()
	end, { description = "Subir brillo", group = "teclado" }),

	awful.key({}, "XF86MonBrightnessDown", function()
		awful.spawn("brightnessctl set 5%-")
		update_brightness()
	end, { description = "Bajar brillo", group = "teclado" }),
	-- }}
	...
	awful.key({ modkey }, "Escape", function() awful.spawn("xsecurelock")
